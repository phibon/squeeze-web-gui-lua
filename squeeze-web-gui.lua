...
function SqueezeserverHandler:_response()
	...
	if logfile then
		logfile:close()
		t['p_log'] = util.capture('tail /var/log/squeezeboxserver/server.log')
	end
...
function ShutdownHandler:post()
	local force = self:get_argument("halt_force", false) or self:get_argument("reboot_force", false)
	if self:get_argument("halt", false) or self:get_argument("halt_force", false) then
		log.debug("halt")
		util.execute("sudo sp-halt" .. (force and " -f" or ""))
	end
	if self:get_argument("reboot", false) or self:get_argument("reboot_force", false) then
		log.debug("restart")
...
function LogHandler:get(log)
	...
	if log == 'squeezelite' then
		...
		--file = "/home/adrian/squeezelite/squeezelite/squeezelite.log"
